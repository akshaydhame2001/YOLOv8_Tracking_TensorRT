            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    def click_event(event, x, y, flags, params):
                        global clicked_class_id
                        # Check if the left mouse button was clicked
                        if event == cv2.EVENT_LBUTTONDOWN:
                            print(f'New click at ({x}, {y})')
                            coordinates[:] = [x, y]
                            print(x,y)
                            ...
                            for output in outputs[i]:
                                ...
                                if bbox[0] < x < bbox[2] and bbox[1] < y < bbox[3]:
                                    # Store the class and id of the clicked bounding box
                                    clicked_class_id = (output[5], output[4])
                                    break
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if coordinates != [] and clicked_class_id == (cls, id):
                            c_x = bbox_left + int(bbox_w/2)
                            c_y = bbox_top + int(bbox_h/2)

                            print(f"center coordinate {c_x}, {c_y}")

                            errorTilt  = 480/2 - c_y
                            errorPan = 640/2 - c_x

                            print(f"errorTilt : {errorTilt}, errorPan : {errorPan}")
